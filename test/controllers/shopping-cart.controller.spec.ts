...
describe('Shopping cart controller', () => {
  ...
  it('should get updated total_price', async () => {
    const login = await request(app.getHttpServer())
      .post('/users/login')
      .send({ username: mockedUser.username, password: mockedUser.password });
    ...
    const response = await request(app.getHttpServer())
      ...
      .send({ total_price: part.price })
      .set('Cookie', login.headers['set-cookie']);

    expect(response.body).toEqual({ total_price: part.price });
  });
  it('should delee cart item', async () => {
    const login = await request(app.getHttpServer())
      .post('/users/login')
      .send({ username: mockedUser.username, password: mockedUser.password });
  ...
  it('should delee all cart item', async () => {
    const login = await request(app.getHttpServer())
      .post('/users/login')
      .send({ username: mockedUser.username, password: mockedUser.password });
      const user = await usersService.findOne({
